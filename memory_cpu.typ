...
To illustrate, we consider a simple square mesh composed of X triangular elements. The lower triangular part of the original symmetric adjacency matrix is shown in REF, while the upper triangular part represents the updated matrix after applying the reverse Cuthill-McKee algorithm. The bandwidth of the resulting matrix is much smaller, meaning that left and right cell indices of each interface are now closer in memory. \
However, REF shows that these reordered meshes do not lead to reduced execution times. This is because, while spatial locality is improved, temporal locality is simultaneously degraded. The colors of each entry in the adjacency matrices represent the corresponding edge indices, ranging from blue (low indices) to rose (high indices). In the initial lower matrix, we observe a smooth gradient from left to right. This is linked to how GMSH @GMSH, the mesh generator used to produce the mesh files feeding Watlab, numbers the interfaces. A closer look at REF reveals that it first numbers boundary edges in a counterclockwise manner. Then, inner interfaces are ordered by the left cell index and, for edges sharing the same left index, by the right cell index. As interfaces are processed in order, this improves temporal locality by increasing the likelihood of accessing common neighboring cells consecutively. \
After reordering the cells, we lose this benefit: the new indices disrupt the original sorting, as shown by the shuffled colors in the updated adjacency matrix of REF. The solution, however, is quite straightforward: we can simply renumber the interfaces by sorting them based on the new left and right indices, using an algorithm like Quicksort @quicksort. \
