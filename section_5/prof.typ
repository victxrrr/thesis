...
We define the serial version of the code as a version of Watlab without any form of parallelism. As a side note, we clarify that this version does not correspond to the original one received at the beginning of the master thesis. On one hand, we corrected some minor bugs in the first weeks, including memory leaks, output irregularities, and logic inconsistencies. On the other hand, the original version still contained remnants of parallelism and synchronization for output tasks, except for pictures. We believe that a fully sequential, cleaner version that is also closer to the parallel implementations presented later is more relevant for comparison and assessment of our work's efficiency. Therefore, we refer to this version when speaking about the serial version. It fully follows the execution diagram of @hydroflow.
...
#figure(
  ...
  box(image("../img/toce_8.png"), clip: true, inset: (left: ins, right: ins)),
  box(image("../img/toce_17.png"), clip: true, inset: (left: ins, right: ins)),
  box(image("../img/toce_51.png"), clip: true, inset: (left: ins, right: ins))
  )
  ,
