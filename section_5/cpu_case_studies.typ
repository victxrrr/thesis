...
#figure(placement: auto,
  box(image("../img/bench_cpu.svg"), stroke: none,
    clip: true,
    inset: (top: -9pt, bottom: -9pt, left: -12pt, right: -10pt)),
  caption: [
...
#figure(placement: auto,
  ...
  caption: [
    Comparison of `HydroFluxLHLLC` roofline analysis
  ]
)<flux_RL>
...
#figure(placement: auto,
  box(
    ...
    clip: true,
    inset: (top: -9pt, bottom: -9pt, left: -10pt, right: -32pt),
    stroke: none
  ),
  caption: [
...
#figure(
  ...
  gap: .75em,
  caption: [Speedup factors with respect to the serial implementation
],
)<cpu_SU>
...
Due to technical constraints, the number of cores that comprise contemporary multicore processors is limited, thereby constraining the achievable speed-up of CPU-parallelized programs in accordance with Ahmdal's law (@amdahl). At the time of this writing, the latest generation of Intel Core i9 processors for consumers has a maximum of 24 cores. Consequently, to further reduce execution times, it is necessary to explore alternative hardware solutions, such as GPUs or FPGAs, which have emerged in the HPC field over the past decades @Vestias.

We decided to develop a GPU implementation of Watlab to pursue the work initiated in the previous thesis @Gamba and leverage the widespread presence of GPUs in consumer computers, originally designed for gaming. A functional and efficient GPU-enabled program would benefit many Watlab users.
