...
#import fletcher.shapes: house, hexagon, pill, chevron, rect, diamond
 ...
 As introduced in the state of the art, various solutions exist to enable an existing application to use GPUs.
 To choose the most appropriate framework for our project, it was necessary to first clearly define the objectives of the future GPU port. The most important are listed below.
...
/ _#unbold[Simplicity]_: Since most Watlab maintainers are hydraulics experts rather than full-time developers, it is crucial to avoid low-level hardware-specific instructions and maintain a high level of abstraction. This ensures a focus on physical modeling and guarantees modularity for future hydraulic features. Writing the GPU version in pure C++ would further simplify the architecture.
